...
> 提示：若在「基础设置」中关闭了「周期记分」模块，或关闭了「启用班委记分」，则不再显示角色选择，锁屏页直接进入管理员密码登录（无班委角色）。
...
| 删除周期 | 仅进行中的周期可删除（已结束周期不允许删除），删除会同时回退学生在周期内的积分；若组内还有待审批记录，确认框会提示数量 |

> 周期时间范围**必填且不能与其他周期重叠**，系统会自动校验；**进入页面时自动结束已过期的进行中周期**（仅管理员侧触发）。
> 编辑周期时间范围时会有二次确认：普通记分记录是按时间范围归属周期的，改范围会重新划分这些历史记录的归属，分析与周期记录的数字可能变化（班委记录按周期归属，不受影响）。
...
3. **班委提交时**：按钮为「提交审批」，提交后提示「尚未计入积分」，需管理员审批（见 8.3）；教师本人记分则直接生效

![规则记分弹窗](docs/images/monitor-record.png)

### 8.3 班委记分审批（仅管理员）

> 前置条件：基础设置 → 「启用班委记分」为开启状态（见 8.6）。关闭后本节的入口与班委账号管理都会隐藏。

默认**开启**。班委在周期内提交的记分不会立即计入学生积分，先进入「待审批」，管理员审批通过后才真正入库。

**开关位置**：基础设置 → 「班委记分需管理员审批」与「启用班委记分」（见 14.4）。关闭审批开关后班委提交直接生效（等同旧行为）；已存在的待审批记录不受影响，仍可正常审批。

**审批入口**：「周期记分」页工具栏的「待审批记录（N）」按钮，N 为待审批条数（有值时按钮转为红色强调）。

**审批弹窗**：

- 顶部统计 待审批 / 已通过 / 已驳回 条数；筛选支持 状态（默认只看待审批）、周期、班委、学生姓名或 ID
- 表格列：提交时间 / 周期 / 原始提交人 / 操作人 / 学生 / 规则 / 次数 / 单次分值 / 合计 / 状态 / 操作
- 单条操作：**编辑**、通过、驳回（驳回必须填写理由）
- 批量操作：勾选多条 → 批量通过 / 批量驳回（统一填一个理由）
- 仅「待审批」记录可被操作，已通过 / 已驳回为终态

**审批时修改**：管理员可在「编辑」中修改 学生 / 规则 / 次数 / 本次分值（改后重算积分；只改次数时按原单次分值等比换算）。**一旦修改，该条记录的「操作人」变为管理员**，原始提交班委仍保留在「原始提交人」列。修改不改变审批状态，改完仍是待审批，再由管理员选择通过或驳回。
...
| ------------------------ | ------------------------------ |
| 班委提交                 | 该班委账号                     |
| 管理员直接通过（未改内容） | 仍为该班委（可统计老师改了多少） |
| 管理员审批时修改过内容     | 变为「管理员」，列表标「管理员已修改」 |
| 管理员驳回               | 仍为该班委                     |

每条记录都记录提交班委的账号名快照，班委账号被删除后仍可追溯。

**口径与规则**：

- 待审批 / 已驳回的记分**不计入**学生积分，也不参与数据分析与 Excel 导出（避免图表与真实积分对不上）
- 周期结束后仍可审批此前提交的待审批记录；但对**已结束周期**的记录通过时会有额外警告（通过会改变该周期的积分成绩，若已按周期结算/发奖会前后不一致）
- 驳回为终态：不会退回给班委重提，也不可撤回
- 删除周期时只回退已通过记录的积分，未入账的待审批 / 已驳回记录不会被误扣
- **审批入口不会消失在没有处理完的记录上**：即使关闭了「启用班委记分」，只要还有待审批记录，入口仍会显示（避免旧记录卡死无人可处理）

### 8.4 周期记录

点「周期记录」按钮，查看当前周期内所有记分明细，展示「规则分值 × 次数 = 总积分」，支持按时间范围、审批状态过滤；开启周期记分时列表会额外展示「周期」「操作人」「审批状态」列（驳回记录悬浮可看驳回理由）。

> 「审批状态」列只在列表里**确存在班委提交的记录**时才出现（这是「真的走过审批」的唯一情形）；教师自己记分、全量操作汇总记录、以及升级前的旧数据都不展示审批状态，统一显示「—」，避免凭空多出一个「已通过」。

### 8.5 学生详情统计

学生表格中点「详情」按钮，弹窗按规则展示该学生在当前周期内的 **已通过次数 / 已通过积分 / 待审批 / 已驳回**，其中只有已通过部分计入学生积分。

### 8.6 班委账号管理（仅管理员）

> 前置条件：基础设置 → 「启用班委记分」为开启状态；关闭后本节按钮隐藏（仅管理员可周期记分）。

管理员在「周期记分」页点「班委账号」按钮，管理本班级的班委账号：

- **新增账号**：输入账号名 + 密码（密码至少 4 位，加密存储）
- **改密码**：重置某账号的密码（无需旧密码）
- **删除账号**：删除班委账号；**已登录的旧会话会在下次提交时被拒绝并提示重新登录**（不会让他继续记分）
...
- 无法审批任何记录（审批仅管理员），也无权处理被驳回的记分（驳回即终态）
- **开关联动**：若管理员关闭「启用班委记分」或「周期记分」模块，班委侧标签页会隐藏并提示「班委记分已被管理员关闭」，同时后续提交会被拒绝（需重新登录）
- 点「退出登录」回到主界面重新选择角色
...
- **关闭启用班委记分的连锁影响**：周期记分页隐藏「班委账号」与「待审批记录」入口，锁屏页直接进入管理员登录，仅管理员可周期记分（详见 14.4）

### 14.4 班委记分开关

与「周期记分」模块配套的两个开关（周期记分未开启时自动置灰）：

| 开关 | 开启（默认） | 关闭 |
| --- | --- | --- |
| **启用班委记分** | 可创建班委账号、锁屏页可选「班委登录」、周期记分页显示「待审批记录」入口 | 班委无法登录记分，不建账号，隐藏班委账号与待审批入口，**只能管理员自己周期记分** |
| **班委记分需管理员审批** | 班委提交的记分先入待审批，管理员通过后才计入积分 | 班委提交直接生效（等同旧行为） |

- 两者关系：审批开关依赖班委记分开关，未启用班委记分时审批开关置灰
- 关闭「启用班委记分」会弹二次确认，提醒先处理完未审批的记录（关闭后审批入口隐藏；若确实还有待审批记录，入口会自动保留以免卡死）
- 关闭「班委记分需管理员审批」也会弹二次确认，明确**不会**自动通过已存在的待审批记录
- 班委侧联动：开关关闭后，已登录班委的周期记分 tab 会隐藏（显示「班委记分已被管理员关闭」提示 + 退出登录），即使未退出也无法提交记分
- 两个开关都默认开启；旧数据缺字段时也按开启处理
