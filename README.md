...
| 系统         | 安装包 | 说明                                                |
| ------------ | ------ | --------------------------------------------------- |
| Windows 10+  | `.exe` | 双击安装，按向导完成；每用户安装，不需要管理员权限 |
| macOS 10.13+ | `.dmg` | 打开后将应用拖入「应用程序」文件夹                 |

> **Windows 从旧版本升级**：本版起安装包改用 NSIS 制作（此前为 Squirrel）。
> 如果电脑上装过旧版本，请先卸载旧版本，再运行新安装包，
> 否则控制面板里会同时出现两条「Miao积分管理」卸载项。
> 卸载旧版本与安装新版本都不会删除班级、学生与积分数据（数据保存在 `%APPDATA%\miao-chemistry`）。
