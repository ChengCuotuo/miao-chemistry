	...
	<div class="monitor-list-container">
		...
		<el-dialog :title="`${detailStudent?.name || ''} - 周期规则统计`" v-model="detailDialogVisible" width="560px">
			...
			<el-table :data="detailStats" border max-height="420" size="small">
				...
				<el-table-column prop="recordCount" label="记分次数" width="110" align="center" sortable />
				<el-table-column prop="recordPoints" label="积分" width="110" align="center" sortable>
					<template #default="scope">
						<span :class="scope.row.recordPoints > 0 ? 'text-success' : scope.row.recordPoints < 0 ? 'text-danger' : 'text-muted'">
							{{ scope.row.recordPoints > 0 ? '+' : '' }}{{ scope.row.recordPoints }}
						</span>
					</template>
				</el-table-column>
			</el-table>
...
const handleCycleSubmit = () => {
	cycleFormRef.value?.validate(async (valid) => {
		...
		if (id) {
			const origin = cycleList.value.find(item => item.id === id);
			const rangeChanged = !!origin && (origin.startTime !== startTime || origin.endTime !== endTime);
			if (rangeChanged) {
				try {
					await ElMessageBox.confirm(
						'修改时间范围会重新归属该时间段内的普通记分记录（周期记录按周期归属，不受影响），数据分析的趋势与周期记录数字可能变化。确认保存？',
						'修改周期时间范围',
...
const detailStats = computed(() => {
	...
	return rules.value.map(rule => {
		...
		return {
			ruleId: rule.id,
			ruleName: rule.name,
			recordCount: ruleRecords.reduce((acc, cur) => acc + (cur.count || 1), 0),
			recordPoints: ruleRecords.reduce((acc, cur) => acc + cur.points, 0),
		};
