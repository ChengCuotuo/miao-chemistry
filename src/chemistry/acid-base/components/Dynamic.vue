...
const draw = () => {
  if (!ctx) return
  // 清除画布
  ctx.clearRect(0, 0, experimentCanvas.value.width, experimentCanvas.value.height)
  // 绘制背景
  drawBackground()
  // 绘制滴管
  drawDropper()
...
const drawBackground = () => {
  ...
  ctx.fillStyle = '#fff'
  ctx.fillRect(0, 0, experimentCanvas.value.width, experimentCanvas.value.height)
...
const drawDropper = () => {
  ...
  const { x, y, width, bodyHeight, tipHeight, tipWidth, liquidHeight } = dropper.value
  ...
  if (
    ...
    ctx.fillRect(x - width / 2 + 2, dropperLiquidY, width - 4, dropperLiquidHeight)
...
const drawDrops = () => {
  ...
  for (let i = 0; i < experiment.value.drops.length; i++) {
    ...
    ctx.beginPath()
    ctx.ellipse(drop.x, drop.y, drop.radius, drop.radius * 1.2, 0, 0, Math.PI * 2)
    ctx.fill()
...
const updateDrops = (timestamp) => {
  ...
  if (exp.isRunning && !exp.isPaused) {
    if (timestamp - exp.lastDropTime > exp.dropInterval) {
      ...
      const newDrop = {
        x: dropper.value.x,
        y: dropper.value.y + dropper.value.bodyHeight + dropper.value.tipHeight + 25,
        radius: 4,
  ...
  for (let i = exp.drops.length - 1; i >= 0; i--) {
    ...
    if (drop.active) {
      ...
      if (drop.y > beakerTopY && drop.y < beakerBottomY) {
        ...
        const currentWidth =
          beakerInfo.baseWidth + (beakerInfo.width - beakerInfo.baseWidth) * (1 - heightRatio * 0.7)
        ...
        if (Math.abs(relativeX) < currentWidth / 2) {
          ...
          // 增加液体量
          if (exp.isRunning && !exp.isPaused) {
            const increment = exp.fillSpeed * (deltaTime / 1000) * 100 * (exp.speedLevel + 1) * 0.25
