<template>
  <div
    :class="['relative overflow-hidden h-full w-full', props.containerClass]"
  >
    <canvas
      ref="canvasRef"
...
const props = withDefaults(
  defineProps<
    Partial<PortalParams> & { class?: string; containerClass?: string }
  >(),
  {
    portalComplexity: 4,
    crystalCount: 12,
...
function initThreeJS() {
  ...
  for (let i = 0; i < 6; i++) {
    const light = new THREE.PointLight(
      new THREE.Color(lightColors[i % 4]),
      0.8,
      20,
    );
    scene.add(light);
...
function addPortalShader(material: any) {
  ...
  material.onBeforeCompile = (shader: any) => {
    ...
    shader.uniforms.portalColor = {
      value: new THREE.Color(params.value.accentColor),
    };
...
function createVortexRings() {
  ...
  for (let ring = 0; ring < 5; ring++) {
    ...
    const mat = new THREE.MeshPhysicalMaterial({
      ...
      emissive: new THREE.Color(colors[ring % colors.length]).multiplyScalar(
        0.2,
      ),
    });
